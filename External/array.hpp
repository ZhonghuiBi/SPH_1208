...
#else
    // Check
    if (SzFactor<1.0) std::cout<<("Array::Resize: SzFactor==%f must be greater than 1.0", SzFactor);
...
{
#ifndef NDEBUG
    if (IdxToRemove<0 || IdxToRemove>=Size()) std::cout<<("Array::DelItem: Subscript==%zd (size==%zd) is out of range.", IdxToRemove, Size());
#endif
...
{
    if (Idxs.Size()>Size()) std::cout<<("Array::DelItems: Array 'Idxs' with indices to be deleted must have size smaller than or equal to %zd. Idxs.Size()==%zd is invalid.",Size(),Idxs.Size());
    Array<Value_T> tmp((*this));
...
inline void Array<Value_T>::DelVal (Value_T const & V)
{
	...
	if (idx_to_remove < 0) std::cout << "Array::DelVal: Value cannot be found for deletion" << std::endl;
...
inline Value_T & Array<Value_T>::operator[] (size_t i)
{
	...
	{
		char str[256];
		sprintf(str, "Array::operator[] (write) Subscript==%zd (size==%zd) is out of range.", i, Size());
...
inline Value_T const & Array<Value_T>::operator[] (size_t i) const
{
	...
	{
		char str[256];
		sprintf(str, "Array::operator[] (read) Subscript==%zd (size==%zd) is out of range.", i, Size());
		std::cout << str << std::endl;
	}
#endif
    return _values[i];
...
{
#ifndef DNDEBUG
    if (&R==this) std::cout<<"Array::operator= The right-hand-size of this operation (LHS = RHS) must not be equal to the LHS." << std::endl;
#endif
...
/** Outputs an array. */
//template<typename Value_T>
//std::ofstream & operator<< (std::ofstream & os, const Array<Value_T> & V)
//{
