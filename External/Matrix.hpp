...
public:
	...
	T& operator()(int row, int col) const;					//括号操作符重载，用于获取矩阵第row行第col列元素 
	...
	//Matrix<T>& operator=(const T a00, const T a01, const T a02, const T a10, const T a11, const T a12, const T a20, const T a21, const T a22);
...
typedef Matrix<double>	Mat3_t;

template  <class T>
Matrix<T>& Matrix<T>::set_to_zero()
{
...
//template  <class T>
//Matrix<T>& Matrix<T>::operator=(const T a00, const T a01, const T a02, const T a10, const T a11, const T a12, const T a20, const T a21, const T a22)
//{
//	//一定要创建临时矩阵，防止同一矩阵赋值（重新分配内存的同时，已经将此矩阵改变！！！！！！）  
//	if (this == &matrix)
//	{
