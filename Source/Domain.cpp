...
void Domain::Periodic_X_Correction(Vec3_t & x, double const & h, Particle * P1, Particle * P2)
{
...
void Domain::Kernel_Set(Kernels_Type const & KT)
{
	KernelType = KT;
	if (KernelType == 2) Cellfac = 3.0; else Cellfac = 2.0;
}

void Domain::Viscosity_Eq_Set(Viscosity_Eq_Type const & VQ)
{
	VisEq = VQ;
}

void Domain::Gradient_Approach_Set(Gradient_Type const & GT)
{
	GradientType = GT;
}

void Domain::AdaptiveTimeStep()
{
	...
	if (deltat < (deltatint / 1.0e5))
		std::cout << ("Too small time step, please choose a smaller time step initially to make the simulation more stable");
}

void Domain::AddSingleParticle(int tag, Vec3_t const & x, double Mass, double Density, double h, bool Fixed)
{
	Particles.Push(new Particle(tag, x, Vec3_t(0, 0, 0), Mass, Density, h, Fixed));
}

void Domain::AddBoxNo(int tag, Vec3_t const & V, size_t nx, size_t ny, size_t nz, double r, double Density, double h, int type, int rotation, bool random, bool Fixed)
{
	if (!(type == 0 || type == 1))
	{
...
void Domain::AddBoxLength(int tag, Vec3_t const & V, double Lx, double Ly, double Lz, double r, double Density, double h, int type, int rotation, bool random, bool Fixed)
{
	if (!(type == 0 || type == 1))
	{
		std::cout << "Packing Type is out of range. Please correct it and run again" << std::endl;
...
void Domain::DelParticles(int const & Tags)
{
...
#pragma omp parallel for schedule(static) num_threads(Nproc)
	...
	if (idxs.Size() < 1) std::cout << ("Domain::DelParticles: Could not find any particles to delete");
	Particles.DelItems(idxs);

	std::cout << "\n" << "Particle(s) with Tag No. " << Tags << " has been deleted" << std::endl;
}

void Domain::CheckParticleLeave()
{
...
void Domain::CellInitiate()
{
	...
	if (!BC.Periodic[0]) { TRPR(0) += hmax / 2;	BLPF(0) -= hmax / 2; }
	else { TRPR(0) += R; BLPF(0) -= R; }
	if (!BC.Periodic[1]) { TRPR(1) += hmax / 2;	BLPF(1) -= hmax / 2; }
	else { TRPR(1) += R; BLPF(1) -= R; }
	if (!BC.Periodic[2]) { TRPR(2) += hmax / 2;	BLPF(2) -= hmax / 2; }
...
void Domain::MainNeighbourSearch()
{
	int q1;
...
void Domain::YZPlaneCellsNeighbourSearch(int q1)
{
	int q3, q2;
...
void Domain::StartAcceleration(Vec3_t const & a)
{
#pragma omp parallel for schedule (static) num_threads(Nproc)
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		if (Particles[i]->IsFree)
		{
			...
			if (Particles[i]->Material > 1 && Particles[i]->TI > 0.0)
			{
				...
				else
				{
					...
					MatVec::Mult(Vec, Val, temp);
					MatVec::Mult(temp, VecT, Particles[i]->TIR);
				}
			}
		}
		else
		{
			...
			MatVec::set_to_zero(Particles[i]->Sigma);
			MatVec::set_to_zero(Particles[i]->ShearStress);
		}
		...
		MatVec::set_to_zero(Particles[i]->StrainRate);
		MatVec::set_to_zero(Particles[i]->RotationRate);
		Particles[i]->S = 0.0;
	}
}

void Domain::PrimaryComputeAcceleration()
{
#pragma omp parallel for schedule (static) num_threads(Nproc)
	for (size_t k = 0; k < Nproc; k++)
	{
		...
		// Reference: S. Adami, X.Y. Hu, et. A generalized wall boundary condition for smoothed particle hydrodynamics
		for (size_t a = 0; a < FSMPairs[k].Size(); a++)
		{
			...
			K = Kernel(Dimension, KernelType, xij.norm() / h, h);
			...
			if (!Particles[P1]->IsFree)
			{
				...
				if (Particles[P1]->Material < 3)	
					Particles[P1]->Pressure += Particles[P2]->Pressure * K + Gravity.dot(xij)*Particles[P2]->Density*K;
				if (Particles[P1]->Material > 1)	
					Particles[P1]->Sigma = Particles[P1]->Sigma + K * Particles[P2]->Sigma;
				if (Particles[P1]->NoSlip)		
			...
			else
			{
				...
				if (Particles[P2]->Material < 3)
					Particles[P2]->Pressure += Particles[P1]->Pressure * K + Gravity.dot(xij)*Particles[P1]->Density*K;
				if (Particles[P2]->Material > 1)
					Particles[P2]->Sigma = Particles[P2]->Sigma + K * Particles[P1]->Sigma;
				if (Particles[P2]->NoSlip)
		...
		if (FSI)
		{
			for (size_t a = 0; a < NSMPairs[k].Size(); a++)
			{
				...
				if (Particles[P1]->Material*Particles[P2]->Material == 2 && (Particles[P1]->IsFree*Particles[P2]->IsFree))
				{
					...
					if (Particles[P1]->Material == 1)//P1: Fluid
					{
						omp_set_lock(&dom_lock);
						...
						Particles[P2]->NSv += Particles[P1]->v * K;
						Particles[P2]->FSIPressure += Particles[P1]->Pressure * K + Gravity.dot(xij)*Particles[P1]->Density*K;
						omp_unset_lock(&Particles[P2]->my_lock);
					...
					else //P1: Solid
					{
						...
						Particles[P1]->FSIPressure += Particles[P2]->Pressure * K + Gravity.dot(xij)*Particles[P2]->Density*K;
						omp_unset_lock(&Particles[P1]->my_lock);
					}
...
#pragma omp parallel for schedule (static) num_threads(Nproc)
	for (size_t i = 0; i < FixedParticles.Size(); i++)
		if (Particles[FixedParticles[i]]->SumKernel != 0.0)
		{
			...
			if (Particles[a]->Material < 3)	
				Particles[a]->Pressure = Particles[a]->Pressure / Particles[a]->SumKernel;
			if (Particles[a]->Material > 1) 
				Particles[a]->Sigma = 1.0 / Particles[a]->SumKernel*Particles[a]->Sigma;
			if (Particles[a]->NoSlip)	
				Particles[a]->NSv = Particles[a]->NSv / Particles[a]->SumKernel;
			...
			if (Particles[a]->Material > 1 && Particles[a]->TI > 0.0)
			{
				...
				else
				{
					...
					MatVec::Mult(Vec, Val, temp);
					MatVec::Mult(temp, VecT, Particles[a]->TIR);
				}
			}
		}
	...
	if (SWIType != 2)
	{
		...
		for (size_t i = 0; i < Particles.Size(); i++)
		{
			...
			{
				/*
				RefDensity: Reference Density of Particle
				RhoF: Density of water or any other fluids
				Density: Density of the particle n+1
				Densitya: Density of the particle n+1/2 (Leapfrog)
				Densityb: Density of the particle n-1 (Modified Verlet)
				*/
...
void Domain::LastComputeAcceleration()
{
#pragma omp parallel for schedule (static) num_threads(Nproc)
	for (size_t k = 0; k < Nproc; k++)
	{
		// SMPairs: The same material free particle pairs
		...
		// FSMPairs: The same material free-fixed particle pairs
    ...
    #pragma omp parallel for schedule (static) num_threads(Nproc)
	//NSMPairs: FSI or water-soil interaction particle pairs
...
#pragma omp parallel for schedule (static) private(test) num_threads(Nproc)
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		if (Particles[i]->IsFree)
		{
			test = sqrt(Particles[i]->h / Particles[i]->a.norm());
...
void Domain::InFlowBCFresh()
{
	...
	if (BC.inoutcounter == 0)
	{
		if (BC.InOutFlow == 1 || BC.InOutFlow == 3)
		{
			if (!(BC.inv.norm() > 0.0) || !(BC.inDensity > 0.0))
			{
...
#pragma omp parallel for schedule(static) private(vel,den) num_threads(Nproc)
		for (size_t i = 0; i < BC.OutPart.Size(); i++)
		{
			...
			if (BC.outv.norm() > 0.0 || temp11 != 1.0)
			{
...
void Domain::InitialChecks()
{
	...
	if (BC.InOutFlow > 0 && BC.Periodic[0])
		std::cout << ("Periodic BC in the X direction cannot be used with In/Out-Flow BC simultaneously");
...
void Domain::TimestepCheck()
{
	...
	if (deltatint > std::min(t1, t2))
		std::cout << ("Please decrease the time step to the allowable range");
}

void Domain::Solve(double tf, double dt, double dtOut, std::string const TheFileKey, size_t maxidx)
{
	std::cout << "\n--------------Solving---------------------------------------------------------------" << std::endl;
	...
	if (!TheFileKey.empty())
	{
		WriteParaview(TheFileKey + "_Initial");
	...
	while (Time < tf && idx_out <= maxidx)
	{
		...
		if (Time >= tout)
		{
			if (!TheFileKey.empty())
			{
				WriteParaview(TheFileKey + "_" + std::to_string(idx_out));
...
void Domain::PrintInput(std::string const FileKey)
{
	//type definition to shorten coding
	std::ofstream oss;

	oss.open(FileKey + "_log.dat", std::ios::out);
	...
	oss << "\nCell Size in XYZ Directions = " << CellSize(0) << CellSize(1) << CellSize(2) << " m\n";
	...
	oss << "\nExternal Acceleration (Gravity enabled)= " << Gravity(0) << Gravity(1) << Gravity(2) << " m/s2\n";
...
void Domain::WriteParaview(std::string const FileKey)
{
	//type definition to shorten coding
	std::ofstream oss;
	...
	//String fn(FileKey);
	//fn.append(".vtk");
	oss.open(FileKey + ".vtk", std::ios::out);
...
#pragma omp parallel for schedule (static) private(P1,P2,P3) num_threads(Nproc)
	...
	oss << FileKey << std::endl;
	oss << "ASCII" << std::endl;
	oss << "DATASET STRUCTURED_GRID" << std::endl;
	oss << "DIMENSIONS " << 20 << "\t" << 20 << "\t" << 20 << std::endl;
	oss << "POINTS " << Particles.Size() << "\t" << "float" << std::endl;
	// Write the coordinate of the particles
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		oss << Posvec[3 * i] << "\t" << Posvec[3 * i + 1] << "\t" << Posvec[3 * i + 2] << std::endl;
	}
	oss << "POINT_DATA" << "\t" << Particles.Size() << std::endl;

	// Write the velocity of the particles
	oss << "VECTORS" << "\t" << "velocity" << "\t" << "float" << std::endl;
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		oss << Velvec[3 * i] << "\t" << Velvec[3 * i + 1] << "\t" << Velvec[3 * i + 2] << std::endl;
	}

	// Write the Acceleration of the particles
	oss << "VECTORS" << "\t" << "Acceleration" << "\t" << "float" << std::endl;
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		oss << ACCvec[3 * i] << "\t" << ACCvec[3 * i + 1] << "\t" << ACCvec[3 * i + 2] << std::endl;
	}

	// Write the stress of the particles
	oss << "TENSORS" << "\t" << "Stress" << "\t" << "float" << std::endl;
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		oss << Sigma[6 * i] << "\t" << Sigma[6 * i + 1] << "\t" << Sigma[6 * i + 2] << std::endl;
		oss << Sigma[6 * i + 1] << "\t" << Sigma[6 * i + 3] << "\t" << Sigma[6 * i + 4] << std::endl;
		oss << Sigma[6 * i + 2] << "\t" << Sigma[6 * i + 4] << "\t" << Sigma[6 * i + 5] << std::endl;
	}

	// Write the strain of the particles
	oss << "TENSORS" << "\t" << "Strain" << "\t" << "float" << std::endl;
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		oss << Strain[6 * i] << "\t" << Strain[6 * i + 1] << "\t" <<Strain[6 * i + 2] <<std::endl;
		oss << Strain[6 * i + 1] << "\t" << Strain[6 * i + 3] << "\t" << Strain[6 * i + 4] << std::endl;
		oss << Strain[6 * i + 2] << "\t" << Strain[6 * i + 4] << "\t" << Strain[6 * i + 5] << std::endl;
	}

	// Write the density of the particles
	oss << "SCALARS" << "\t" << "density" << "\t" << "float" << std::endl;
	oss << "LOOKUP_TABLE" << "\t" << "default" << std::endl;
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		oss << Density[i] << std::endl;
	}

	// Write the density of the particles
	oss << "SCALARS" << "\t" << "Pressure" << "\t" << "float" << std::endl;
	oss << "LOOKUP_TABLE" << "\t" << "default" << std::endl;
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		oss << Pressure[i] << std::endl;
	}

	// Write the Mass of the particles
	oss << "SCALARS" << "\t" << "Mass" << "\t" << "float" << std::endl;
	oss << "LOOKUP_TABLE" << "\t" << "default" << std::endl;
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		oss << Mass[i] << std::endl;
	...
	// Write the sh of the particles
	oss << "SCALARS" << "\t" << "sh" << "\t" << "float" << std::endl;
	oss << "LOOKUP_TABLE" << "\t" << "default" << std::endl;
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		oss << sh[i] << std::endl;
	...
	// Write the sh of the particles
	oss << "SCALARS" << "\t" << "Tag" << "\t" << "float" << std::endl;
	oss << "LOOKUP_TABLE" << "\t" << "default" << std::endl;
	for (size_t i = 0; i < Particles.Size(); i++)
	{
		oss << Tag[i] << std::endl;
...
void Domain::CalcForce11(Particle * P1, Particle * P2)
{
	...
	if ((rij / h) <= Cellfac)
	{
		...
		if (Alpha != 0.0 || Beta != 0.0)
		{
			...
			if (vij.dot(xij) < 0)
				PIij = (-Alpha*0.5*(Ci + Cj)*MUij + Beta*MUij*MUij) / (0.5*(di + dj));		///<(2.74) Li, Liu Book
		}
		...
		if (P1->TI > 0.0 || P2->TI > 0.0)
		{
			...
			{
				if (P1->Pressure < 0.0) 
					Ri = -P1->Pressure / (di*di);
				if (P2->Pressure < 0.0) 
					Rj = -P2->Pressure / (dj*dj);
			}
		...
		if ((P1->NoSlip || P2->NoSlip) || (P1->IsFree*P2->IsFree))
		{
			...
			else
			{
				...
				if (P1->IsFree)	
					vab = P1->v - (2.0*P2->v - P2->NSv); 
				else 
					vab = (2.0*P1->v - P1->NSv) - P2->v;
				if (!P1->IsFree) 
					Mu = P2->Mu;
				if (!P2->IsFree) 
					Mu = P1->Mu;
		...
		if (P1->IsFree)
		{
			...
			if (SWIType == 1)						
				P1->S = P1->S + mj / dj*vab(0)*xij(1)*-GK;
			P1->ZWab += mj / dj* K;
		}
		else
			P1->ZWab = 1.0;
		...
		if (P2->IsFree)
		{
			...
			if (SWIType == 1)						
				P2->S = P2->S + mi / di*vab(0)*xij(1)*-GK;
			P2->ZWab += mi / di* K;
		}
		else
			P2->ZWab = 1.0;
...
void Domain::CalcForce2233(Particle * P1, Particle * P2)
{
	...
	if ((rij / h) <= Cellfac)
	{
		...
		{
			if (!P1->IsFree) { di = P2->Density;	mi = P1->FPMassC * P2->Mass; }
			else { di = P1->Density;	mi = P1->Mass; }
			if (!P2->IsFree) { dj = P1->Density;	mj = P2->FPMassC * P1->Mass; }
			else { dj = P2->Density;	mj = P2->Mass; }
		}
		else
		{
			if (!P1->IsFree)
		...
		MatVec::set_to_zero(Sigmaj);
		MatVec::set_to_zero(Sigmai);
		Sigmai = P1->Sigma;
		Sigmaj = P2->Sigma;
		...
		if (GradientType == 0)
			MatVec::Mult(GK*xij, (1.0 / (di*di)*Sigmai + 1.0 / (dj*dj)*Sigmaj + PIij + TIij), temp);
		else
			MatVec::Mult(GK*xij, (1.0 / (di*dj)*(Sigmai + Sigmaj) + PIij + TIij), temp);
